...
router.get('/order/:orderToken', asyncHandler(async (req, res) => {
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  let resolvedGalleryIds = allowedGalleryIds && allowedGalleryIds.length > 0
    ? allowedGalleryIds
    : [];
...
router.post('/send-links-email', asyncHandler(async (req, res) => {
...
router.patch('/:id/deliver', asyncHandler(async (req, res) => {
  if (!UUID_RE.test(req.params.id))
    return res.status(400).json({ message: 'Invalid ID format' });
  const order = await ProductOrder.findById(req.params.id);
  if (!order || order.adminId !== req.admin.id)
    return res.status(404).json({ message: 'Order not found' });
  if (order.status !== 'submitted')
    return res.status(409).json({ message: 'Only submitted orders can be marked as delivered' });
...
router.patch('/:id/galleries', asyncHandler(async (req, res) => {
  if (!UUID_RE.test(req.params.id))
    return res.status(400).json({ message: 'Invalid ID format' });

  const { allowedGalleryIds } = req.body;
  if (!Array.isArray(allowedGalleryIds))
    return res.status(400).json({ message: 'allowedGalleryIds must be an array' });
  ...
  if (allowedGalleryIds.length > 0) {
    ...
    if (invalid.length > 0)
      return res.status(422).json({ message: 'One or more gallery IDs are invalid', invalid });
  }
