/**
 * Compression worker handlers.
 *
 * Covers the full JXL transcode pipeline:
 *   compression.transcode — server-side JPEG → JXL via cjxl --lossless_jpeg=1
 *   compression.verify    — browser-uploaded PNG sidecar JXL round-trip check
 *   compression.cleanup   — delete pending-deletion/ safety copy after +7d re-verify
 *
 * Also exports:
 *   assertDjxlVersion / assertCjxlVersion — sync boot version probes
 *   selfTestCjxlJpeg                      — async boot round-trip self-test
 *
 * All handlers are idempotent. See top-level workers/index.js for the
 * concurrency model and pg-boss option placement rules.
 */
